...
class ConfigCreator:
    ...
    def _get_excitation_wavelengths(self) -> List[float]:
        num_channels = len(self._std_meta["ChannelDetails"]["ChannelDetailsArray"])
        channel = self._std_meta["ChannelDetails"]["ChannelDetailsArray"][0]

        if "ExcitationWavelengthNM" in channel:
            exc_wav = []
            for ch in self._std_meta["ChannelDetails"]["ChannelDetailsArray"]:
